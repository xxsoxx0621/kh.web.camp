...
<body>
	...
	<table class="table table-hover" id="table" border="1">
		<td colspan=5 align=center>1:1 문의 리스트</td>
		<tr>
			<td width=50>번호</td>
			<td width=320>문의 내용</td>
			<td width=100>작성자</td>
		...
		<c:forEach var="list" items="${list }">
			<tr>
				<td>${list.qna_no }</td>
				<td><a href="/qnaDetail.qna?seq=${list.qna_no }">${list.qna_title }</a></td>
				<td>${list.qna_id }</td>
				<td>${list.qna_date }</td>
...
$('.delBtn').on('click', () => {
	if(confirm("정말로 삭제하시겠습니까 ?")){
		location.href="/adminQnaDel.admin?seq=${list.qna_no }";		
	}else{
		return;
	}
})
