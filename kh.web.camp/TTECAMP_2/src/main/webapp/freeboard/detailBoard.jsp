...
<body>
	...
	<section id="fullbody">
		...
		<div id="container">
			...
			<form action="/modify.board" id="modifyForm">
				<c:forEach var="detail" items="${list }">
					...
					<div id="user_info">
						<div id="writer">${detail.user_id }</div>
						<div id="writing_date">${detail.date }</div>

					</div>
					<div id="content">
						<textarea name="content" id="content_text" cols="30" rows="10"
							readonly>${detail.contents }</textarea>
					</div>
			</form>
			...
			<div id="comment_container">
				...
				<c:forEach var="rep" items="${replyList }">
					...
					<div id="chatBoxP">
						...
						<div id="chatBox">
							<div id="comment_id">${rep.user_id }</div>
							<div id="comment_content">${rep.contents }</div>
							<div id="comment_date">${rep.date }</div>
							<div id="comment_content_btns">
								<c:if test="${admin != null }">
									<a class="comment_del_btn"
										href="/deleteRpl.rpl?seq=${rep.board_seq }&replyNo=${rep.reply_no }"><button
											id="comment_delete">X</button></a>
								</c:if>
							</div>
						</div>
					</div>
					<div id="chatBox_line"></div>
				</c:forEach>
				...
				<c:choose>
					<c:when test="${loginID != null || admin !=null}">
						...
						<div id="comment">
							<div id="comment_body">
								...
								<form action="/commentAdd.rpl" method=post id="commentAddForm">
									<c:choose>
										<c:when test="${detail.user_id == loginID || admin != null}">
											<input type=hidden value="${detail.user_id }" name="id">
										</c:when>
									...
									<input type=hidden value="${detail.board_seq }" name="seq">
					...
					<c:otherwise>
						...
						<div id="comment" class="notMemberCmt">
							<div id="comment_body">

								<textarea name="comment" id="comment_text" cols="30" rows="10"></textarea>
							</div>
							<div id="comment_add">
								<button id="addComment">등록하기</button>
							</div>
		...
		<div id="bottoms">
			...
			<a href="/boardlist.board?cpage=1">
				<button type="button" class="goToListBtn">목록으로</button>
			</a>
   ...
   $('#delBtn').on('click', () => {
      ...
      location.href="/delete.board?seq=${detail.board_seq}";
      }
   });
   ...
   $('#comment_body').on('click', () =>{
      $('#comment_text').val(" ");
   })
   
   $('.notMemberCmt').on('click',() =>{
      alert("댓글을 남기기위해선 로그인이 필요합니다.");
