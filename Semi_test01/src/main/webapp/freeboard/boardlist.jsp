	...
	<div id="container">
		...
		<div id="contents">
			<c:forEach var="dto" items="${list }">
				<div id="content">
					<div class="num">${dto.board_seq }
						<input type="hidden" name="boardSeq" value="${dto.board_seq }">
					</div>
					<div class="title">
						<a href="/detail.board?seq=${dto.board_seq }&cpage=1">${dto.title }</a>
					...
					<c:choose>
						...
						<c:when test="${dto.user_id == adminID }">
					...
					<div class="date">${dto.detailDate }</div>
		...
		<div id="bottom">
			...
			<!-- 회원 로그인시는 게시글 작성가능하지만, 비회원시에는 작성하기 버튼 클릭시 <로그인이 필요한 페이지 입니다.> 팝업후 로그인 유도 -->
			<c:choose>
				<c:when test="${loginID != null }">
