...
<body>
	...
	<div id="container">
		...
		<form action="/modifyNotice.notice" id="modifyForm" method=post>
			<c:forEach var="detail" items="${noticeDetail }">

				<input type="hidden" value="${detail.notice_no_seq }" name=seq>

				<div id="input_title">
					<input type="text" name="input_title" id="title"
						value="${detail.title }" readonly>
				</div>
				<div id="writer">관리자</div>
				<div id="content">
					...
					<div id="comment_content_btns">
						...
						<c:if test="${admin != null }">
							<a id="comment_del_btn"
								href="/deleteRpl.nrc?seq=${rep.board_seq }&replyNo=${rep.notice_reply_no }"><button
				...
				<c:when test="${loginID != null}">
					<div id="comment">
						<div id="comment_body">
							...
							<form action="/commentAdd.nrc" method=post id="commentAddForm">
								<c:choose>
									<c:when test="${admin != null }">
										<input type=hidden value="${detail.admin_id }" name="id">
									</c:when>
		...
		<div id="bottoms">
			...
			<a href="noticeList.notice?cpage=1"><button type="button">목록으로</button></a>
	...
	<script>
		$('#delBtn').on('click', () => {
			if(confirm("정말로 삭제하시겠습니까 ?")){
				location.href="/deleteNotice.notice?seq=${detail.notice_no_seq }";
			}
		})
