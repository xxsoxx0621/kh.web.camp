...
public class QnaReplyDAO {
	...
	private static  QnaReplyDAO instance = null;
	...
	public int countReply(int seq) throws Exception {
		String sql = "select count(*)COUNT from QNAREPLY where BAORD_SEQ = ?";
		try(
	...
	public int addComment(int seq, String id, String content) throws Exception{
		String sql = "insert into QNAREPLY values(REPLY_NO_QNA_SEQUENCE.NEXTVAL, ?, ?, ?,SYSDATE)";
		try(
				Connection con = this.getConnection();
				PreparedStatement pstat = con.prepareStatement(sql);
				){
	...
	public List<QnaReplyDTO> selectAll(int seq) throws Exception{
		String sql = "select * from QNAREPLY WHERE BOARD_SEQ = ?";
		try(
				Connection con = this.getConnection();
				PreparedStatement pstat = con.prepareStatement(sql);
				){
			...
			try(ResultSet rs = pstat.executeQuery();){
				...
				while(rs.next()) {
					...
					QnaReplyDTO dto = new QnaReplyDTO(reply_no, board_seq, user_id, contents, write_date);
					list.add(dto);	
				}
				return list;
			}
	...
	public int deleteReply(int replyNo, int seq) throws Exception{
		String sql = "delete from QNAREPLY where REPLY_NO = ? and BOARD_SEQ = ?";
		try(
				Connection con = this.getConnection();
				PreparedStatement pstat = con.prepareStatement(sql);
				){
	...
	public int delPageReply(int seq) throws Exception {
		String sql = "delete from QNAREPLY where board_seq = ? ";
		try(
				Connection con = this.getConnection();
				PreparedStatement pstat = con.prepareStatement(sql);
				){
